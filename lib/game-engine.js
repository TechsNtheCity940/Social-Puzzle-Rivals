...
function joinMatch({ revengeMode = false, forcedPuzzleId, forcedVariantIndex } = {}) {
  cleanupMatches();
  const store = readStore();
  const profile = store.profile;
  const puzzle = selectPuzzle(profile.performance, revengeMode, forcedPuzzleId, forcedVariantIndex);
...
function buildRivals(difficulty) {
  return shuffle([...RIVAL_NAMES]).slice(0, 5).map((name, index) => ({
    name,
    mmr: randomNumber(1180, 1630),
    finishTime: randomNumber(20, 62) + difficulty * 5 + index * 2,
    progress: 0,
    status: "waiting",
  }));
}

function selectPuzzle(performance, revengeMode, forcedPuzzleId, forcedVariantIndex) {
  const factories = getPuzzleFactories(forcedVariantIndex);
  if (forcedPuzzleId) {
    const forcedFactory = factories.find((factory) => factory().id === forcedPuzzleId);
    if (forcedFactory) {
      return forcedFactory();
    }
  }

  const weighted = factories.map((factory) => {
    ...
    return { factory, weight: revengeMode ? weakness : 1 };
  });
...
function getPuzzleFactories(forcedVariantIndex = null) {
  return [
    () => createCrosswordMini(forcedVariantIndex),
    () => createAnagramBlitz(forcedVariantIndex),
    () => createMissingLetterDash(forcedVariantIndex),
    () => createLogicSequence(forcedVariantIndex),
    () => createEmojiTrivia(forcedVariantIndex),
    () => createMemoryGrid(),
    () => createRiddleSprint(forcedVariantIndex),
    () => createPatternPulse(forcedVariantIndex),
  ];
}
...
function createMemoryGrid() {
  const answer = shuffle([0, 1, 2, 3, 4, 5, 6, 7, 8]).slice(0, 4).sort((left, right) => left - right);
  return {
    id: "memory-grid",
    name: "Memory Grid",
    category: "Memory",
